...
class DPKMeans(KMeans):
    ...
    # def _calculate_color_noise_covariance(self, X):
    #     """Calculates the differentially private color noise covariance matrix.
    ...
    #     Returns
    #     -------
    #     dp_color_covariance : array-like
    ...
    #     print(
    #         f"epsilon_c: {dp_epsilon_total}, delta_c: {self.dp_delta}, gamma_c: {gamma_c}"
    #     )

    #     # Calculating the cardinality of each cluster
    #     cluster_sum = np.array(
    #         [np.sum(X[self.labels_old == k], axis=0) for k in range(self.n_clusters)]
    #     )
    #     cluster_cardinality = np.bincount(
    #         self.labels_old, minlength=self.n_clusters
    #     ).reshape(-1, 1)
    #     cluster_center_without_node = (cluster_sum[self.labels_old] - X) / (
    #         cluster_cardinality[self.labels_old] - 1
    #     )
    #     modified_cluster_centers = np.tile(self.cluster_centers_old, (X.shape[0], 1, 1))
    #     modified_cluster_centers[
    #         np.arange(X.shape[0]), self.labels_old
    #     ] = cluster_center_without_node
    #     modified_cluster_centers = modified_cluster_centers.reshape(X.shape[0], -1).T
    #     change_in_cluster_centers = (
    #         self.cluster_centers_old.reshape(-1, 1) - modified_cluster_centers
    #     )  # Form the $\bm{C}_{XX'}$ matrix

    #     norm_change_in_cluster_centers = np.linalg.norm(
    #         change_in_cluster_centers, axis=0
    #     )  # Calculate the norm of each column
    #     sorted_norm_indices = np.argsort(
    #         norm_change_in_cluster_centers
    #     )  # Sort the columns of Cxx by column norm
    #     sorted_change_in_cluster_centers = change_in_cluster_centers[
    #         :, sorted_norm_indices
    #     ]  # Store a copy of the sorted columns

    #     _, independent_indices = sympy.Matrix(
    #         sorted_change_in_cluster_centers
    #     ).rref()  # Check which columns are linearly independent
    #     independent_columns = sorted_change_in_cluster_centers[
    #         :, list(independent_indices)
    #     ]
    #     (
    #         independent_columns_U,
    #         independent_columns_D,
    #         independent_columns_V,
    #     ) = np.linalg.svd(independent_columns)
    #     independent_columns_T_root = (
    #         independent_columns_U[
    #             : len(independent_indices), : len(independent_indices)
    #         ]
    #         @ np.diag(np.sqrt(independent_columns_D))
    #         @ independent_columns_V
    #     )

    #     independent_columns_T = np.square(independent_columns_T_root)
    #     lambda_star = np.real(
    #         np.power(
    #             gamma_c
    #             * (
    #                 np.linalg.inv(independent_columns_T)
    #                 @ np.ones((len(independent_columns_T), 1))
    #             ),
    #             -2,
    #         )
    #     )
    #     R_lambda_star = np.sum(
    #         lambda_star[i]
    #         * np.outer(
    #             independent_columns[:, i],
    #             independent_columns[:, i],
    #         )
    #         for i in range(len(independent_indices))
    #     )
    #     R_lambda_star_U, R_lambda_star_D, R_lambda_star_V = np.linalg.svd(R_lambda_star)

    #     dp_color_covariance = (
    #         R_lambda_star_U @ np.diag(np.sqrt(R_lambda_star_D)) @ R_lambda_star_V
    #     )
